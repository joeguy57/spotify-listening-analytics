...
deduplicated as (
    select
        *,
        row_number() over (
            partition by user_id, track_id, ts
...
cleaned as (
    select
        ...
        {{ dbt_utils.generate_surrogate_key(['user_id', 'track_id', 'ts']) }} as event_id,
        user_id,
        track_id,
        ...
        shuffle
    from deduplicated
    where row_num = 1 -- keep only one row per (user_id, track_id, ts) combination
)
